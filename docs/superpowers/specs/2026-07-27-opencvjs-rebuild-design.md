...
| 3   | 单一精简产物（用户侧一个包、一行 import） | 控制维护成本。「精简」= 白名单裁掉 dnn 模块（当前 13.9 MB 的体积大头，数据段含 TensorFlow protobuf），保留 core/imgproc/objdetect/features2d/video/calib3d；不做极限裁剪（见非目标） |
...
- **阶段 0（1.0.1）**：保持现有 API 形状（仍覆盖原生），仅把内部的 cvtColor 往返换成 `.clone()`。现有用户无需改代码即可摆脱 P0 崩溃与性能退化。
...
| 阶段  | 内容                                                            | 是否依赖 wasm 重建 |
| ----- | --------------------------------------------------------------- | ------------------ |
| **0** | 根因 A 修复（cvtColor hack → clone）+ 对应测试。以 1.0.1 发布   | 否，可立即交付     |

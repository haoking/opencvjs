...
// **两个 embind getter**，每次读取都是一次跨语言调用。实测（同一进程内轮换、丢首轮、
// 取最小值，node v22.22.2 / darwin-arm64）：
//
//   replaceMatOnRect 32×32   循环走 rawPtr 176.73 ms  vs  循环调 PTR 362.01 ms  → +105%
//   addOnCol 逐列 64 行       循环走 rawPtr  53.94 ms  vs  循环调 PTR 100.84 ms  → +87%
//
// ── 为什么 region-ops.bench.js 覆盖不到 ──────────────────────────────────────
// 那个门禁测的是 roiClone()，而 roiClone() 内部是「原生 roi + clone」，**根本不经过
// PTR，也不逐像素循环**。把上面这几个方法的循环改回调 PTR，它照样一路绿灯。也就是
// 说，在本文件出现之前，一次 82%–105% 的退化能悄悄合进主干。这正是本项目一直在清的
// 「会说谎的绿灯」，所以补上这个门禁。
//
// ── 门禁的形状（与直觉相反的一点）────────────────────────────────────────────
